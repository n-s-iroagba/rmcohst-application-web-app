...
import {  ApplicationStatus, ApplicationCreationAttributes, Application } from "../models/Application"
...
class ApplicationService {
  ...
  public async createInitialApplication(  applicantUserId: number,data: {
    sessionId: string
    programId?: string 
  }): Promise<Application> {
    try {
      ...
      if (existingApplication) throw new AppError("Application already exists for this user in this academic session.", 409)
     ...
     const programSpecificQualification = await ProgramSpecificQualification.findOne({where:{programId:program.id}})
     if (programSpecificQualification) await ApplicantProgramSpecificQualification.create({applicationId,qualificationType:programSpecificQualification.qualificationType})
 ...
 public async  finalizeApplicantSubmission(
  ...
  if (validationErrors.length > 0) {
    throw new AppError(`Application cannot be submitted due to the following issues: ${validationErrors.join('; ')}`, 400);
  }
