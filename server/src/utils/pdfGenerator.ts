  ...
  async generateAdmissionLetter(data: AdmissionLetterData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Date
        doc.text(`Date: ${new Date(data.dateOfAdmission).toLocaleDateString()}`, { align: 'right' })
           .moveDown(1);

        // Application ID
        doc.text(`Application ID: ${data.applicationId}`)
           .moveDown(2);
        ...
        doc.fontSize(12)
           ...
           .moveDown(1)
           .text(`We are pleased to inform you that you have been offered admission into the ${data.program} program in the ${data.department}, Faculty of ${data.faculty} for the ${data.sessionYear} academic session.`, { align: 'justify' })
           .moveDown(1)
        ...
        // Reporting date
        doc.text(`You are required to report for registration on or before ${new Date(data.reportingDate).toLocaleDateString()}.`, { align: 'justify' })
           .moveDown(1);
        ...
        doc.fontSize(11)
           ...
           .font('Helvetica')
           .text('Failure to report within the stipulated time may result in forfeiture of this admission offer.', { align: 'justify' })
           .moveDown(2);

        // Closing
        doc.text('Once again, congratulations on your admission. We look forward to welcoming you to our institution.')
           .moveDown(2);
        ...
        doc.fontSize(10)
           .fillColor('gray')
           .text('This is a computer-generated admission letter and is valid without signature.', { align: 'center' })
