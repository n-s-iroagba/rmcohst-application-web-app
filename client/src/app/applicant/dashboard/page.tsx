...
const Todo = () => {
  ...
  const paymentUrl = user?API_ROUTES.PAYMENT.GET_CURRENT_SESSION_APPLICATION_PAYMENTS(user.id):null
...
if (!hasPaid) {
  if (hasPending) {
    ...
    todos.push(
      <TodoAlert
        key="no-payment"
        message="You do not have any payments. Add a payment to start managing transactions."
        link="/applicant/programs"
        heading="No Payment Found"
      />

    );
console.log('tTTTTTTTTTTTTT',todos.length)
}
  if (
    authLoading || paymentLoading || sessionLoading||paymentLoading ) {
    return (
  ...
  if (applicationError|| paymentrror || sessionError||authError){
    return (
    <ErrorAlert message={applicationError || paymentrror ||authError||sessionError} />
  );
  }
  ...
  return (
    ...
    <AlertTriangle className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 flex-shrink-0" /> {/* ✅ Lucide */}
  ...
  <h3 className="text-base sm:text-lg font-semibold text-blue-700">
    Applicant Dashboard
        ...
        >
          {todos.length > 0?(todos.map((todo)=>( <motion.div
          ...
          </motion.div>))):    <div className="p-4 sm:p-6 bg-blue-50 rounded-lg sm:rounded-xl border border-blue-200 text-center">
