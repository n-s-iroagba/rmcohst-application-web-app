...
import { handleAddToArrayOfArrays, handleArrayChange, handleChange, handleChangeArrayInArray, handleRemoveFromArrayOfArrays } from "@/helpers/handleChange";
...
export const useApplicationRequirments = () => {
  ...
  const [sscRequirmentsData, setSCCRequirementsData] = useState<SSCSubjectMinimumGradeCreationDto[]>([]);
...
const addProgramSpecificRequirment = (parentIndex: number) => {
 handleAddToArrayOfArrays(parentIndex,setProgramData, 'programSpecificRequirements',tempProgramSpecificRequirement)
};

  const removeProgramSpecificRequirment = (parentIndex: number,  childIndex: number) => {
 handleRemoveFromArrayOfArrays(parentIndex,childIndex,setProgramData, 'programSpecificRequirements')
  };
