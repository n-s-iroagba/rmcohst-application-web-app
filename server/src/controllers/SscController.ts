...
class SscController {
  ...
  static async Update(req: AuthRequest, res: Response): Promise<void> {
    ...
    try {
      let qualification = await ApplicantSSCQualification.findOne({ where: { applicationId: Number(applicationId) } })
      ...
      if (subjectsJSON) {
        ...
        await ApplicantSSCSubject.destroy({ where: { applicantSSCQualificationId: qualification.id } })
      ...
      const finalQualification = await ApplicantSSCQualificationService.getSSCQualificationByApplication(
